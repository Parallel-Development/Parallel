...
exports.run = async(client, message, args) => {
    ...
    if (!args[2] && (option === 'add' || option === 'remove')) return client.util.throwError(message, client.config.errors.missing_argument_role);
    if (!role && (option === 'add' || option === 'remove')) return client.util.throwError(message, client.config.errors.invalid_role);

    const guildAFK = await afkSchema.findOne({ guildID: message.guild.id });
    const { allowedRoles } = guildAFK;

    if (option === 'add') {
        if (allowedRoles.includes(role.id)) return client.util.throwError(message, 'the provided role is already on the afk role whitelist!');
        ...
        if (!allowedRoles.includes(role.id)) return client.util.throwError(message, 'the provided role is not on the afk role whitelist!');
        await afkSchema.updateOne({
            guildID: message.guild.id
        },
            {
        ...
        const _allowedRoles = await afkSchema.findOne({ guildID: message.guild.id }).then(result => result.allowedRoles);
        ...
        const roleList =
            _allowedRoles.map(role => message.guild.roles.cache.get(role.id)).join(' ').length <= 2000
                ...
                : await client.util.createBin(
                    _allowedRoles.map(role => message.guild.roles.cache.get(role.id))
                );
