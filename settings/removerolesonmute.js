...
exports.run = async (client, message, args) => {
        ...
        const removerolesonmuteStateEmbed = new Discord.MessageEmbed()
            ...
            .setDescription(
                removerolesonmute
                    ...
                    : 'All roles will remain with the user when muted, and the muted role will be added'
            );
    ...
    if (args[1].toLowerCase() === 'enable') {
        ...
        return message.reply(
            `${client.config.emotes.success} I will now remove all roles from a user when they are muted and add the muted role to them`
        );
        ...
        return message.reply(
            `${client.config.emotes.success} I will now only add the muted role to a user when they are muted`
        );
    } else {
