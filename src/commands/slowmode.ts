...
@data(
  ...
  .setName('slowmode')
  .setDescription('Modify the slowmode in the channel.')
  ...
  .addStringOption(option => option.setName('method').setDescription('Choose to add or remove to the current slowmode.')
  .addChoices(
    { name: 'Set', value: 'set' },
    { name: 'Add', value: 'add' },
    { name: 'Remove', value: 'subtract'}
  ))
)
class SlowmodeCommand extends Command {
  async run(interaction: ChatInputCommandInteraction<'cached'>) {
    ...
    switch (method) {
      case 'add': slowmode += interaction.channel.rateLimitPerUser ?? 0; break;
      case 'subtract': slowmode = (interaction.channel.rateLimitPerUser ?? 0) - slowmode; break;
    }
