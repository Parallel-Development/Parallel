...
class MessageLogListener extends Listener {
  ...
  async run(oldMessage: Message<true> | null, message: Message<true>, type: 'edit' | 'delete') {
    ...
    if (guild.messageLogIgnoredChannels.includes(message.channel.id))
      return;
    ...
    .setAuthor({ name: `Message from ${message.author.tag} (${message.author.id}) ${type === 'edit' ? 'edited' : 'deleted'}`, iconURL: message.author.displayAvatarURL() })
    .setTimestamp()
    ...
    if (type === 'edit' && message.content.length > 0) {
      embed
        ...
        .addFields(
          {
            name: 'Old',
            value: oldMessage?.content ? oldMessage.content.length > 1000 ? await bin(oldMessage.content) : oldMessage.content : '<unknown>'
          },
          {
            name: 'New',
