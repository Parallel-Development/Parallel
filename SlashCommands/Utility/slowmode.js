    ...
    async execute(client, interaction, args) {
        if (!args['slowmode'])
            return interaction.reply(
                `The current slowmode is \`${client.util.duration(interaction.channel.rateLimitPerUser * 1000)}\``
            );
        let slowmode = args['slowmode'];
        if (!parseInt(slowmode) && parseInt(slowmode) !== 0)
            return client.util.throwError(interaction, 'an invalid slowmode was provided');
