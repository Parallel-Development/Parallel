...
module.exports = {
    ...
    data: new SlashCommandBuilder().setName('clear').setDescription('Bulk delete messages in a channel')
    .addIntegerOption(option => option.setName('amount').setDescription('The amount of messages to be cleared').setRequired(true))
    ...
    async execute(client, interaction, args) {
        ...
        if (amount > 100 || amount < 1) return client.util.throwError(interaction, 'Number must a number between 1-100');
        if (!interaction.channel.permissionsFor(interaction.guild.me).has(Discord.Permissions.FLAGS.MANAGE_MESSAGES)) return client.util.throwError(interaction, client.config.errors.my_channel_access_denied);
