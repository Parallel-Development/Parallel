    ...
    async execute(client, interaction, args) {
        ...
        if (
            !channel
                ...
                .has([Discord.Permissions.FLAGS.MANAGE_MESSAGES, Discord.Permissions.FLAGS.SEND_MESSAGES])
        )
        ...
        if (
            ...
            return client.util.throwError(interaction, client.config.errors.your_channel_access_denied);
        ...
        if (
            !channel
                .permissionsFor(interaction.member)
                .has([Discord.Permissions.FLAGS.MANAGE_CHANNELS, Discord.Permissions.FLAGS.SEND_MESSAGES]) ||
            !channel.permissionsFor(interaction.member).has(Discord.Permissions.FLAGS.SEND_MESSAGES)
        )
            return client.util.throwError(interaction, client.config.errors.your_channel_access_denied);
        ...
        if (!getLockSchema)
            return client.util.throwError(
                interaction,
                'This channel is already unlocked! (If you manually locked, just run the lock command to register this channel as locked)'
            );
            ...
            if (initialPermissionOverwrite) {
                const newPermissionOverwrite = {
                    ...
                    deny: initialPermissionOverwrite.deny.has(Discord.Permissions.FLAGS.SEND_MESSAGES)
                        ...
                        : initialPermissionOverwrite.deny,
        ...
        await channel.permissionOverwrites.set(
            ...
            `Command /unlock ran by ${interaction.user.tag} (${interaction.user.id})`
        );
