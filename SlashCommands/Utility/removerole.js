...
module.exports = {
    ...
    data: new SlashCommandBuilder()
        ...
        .addUserOption(option =>
            option.setName('member').setDescription('The target you are removing the role from').setRequired(true)
        )
        .addRoleOption(option =>
            option.setName('role').setDescription('The role you are removing from the target member').setRequired(true)
        )
        .addBooleanOption(option =>
            option
                .setName('dm')
                .setDescription('DM the user regarding their role change (with if specified, your reason)')
        )
        .addStringOption(option =>
            option
                .setName('reason')
                .setDescription('The message sent to the user regarding their role change if you are to DM them')
        ),
    async execute(client, interaction, args) {
        ...
        if (!member.roles.cache.has(role.id))
            return client.util.throwError(interaction, 'This member does not has this role!');
