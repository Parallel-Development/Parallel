...
module.exports = {
    ...
    data: new SlashCommandBuilder().setName('lockserver').setDescription('Lock all server channels. The bot will ignore channels in which non-mods cannot talk in or view')
        .addBooleanOption(option => option.setName('include_staff_channels').setDescription('Include channels non-moderators cannot talk in or view')
    ),
    ...
    async execute(client, interaction, args) {
        ...
        for (let i = 0; i !== channels.length; ++i) {
            ...
            if(!checkIfLocked && client.guilds.cache.get(interaction.guild.id).channels.cache.get(channel.id)) {
                ...
                if(!allOverwrites.length && everyoneRoleOverwrite); 
                    ...
                    await channel.permissionOverwrites.set(newPermissionOverwrites, `Command /lockserver ran by ${interaction.author.tag} (${interaction.author.id})`).catch(async() => await interaction.editReply('Hmm...'));
        ...
        global.lockdownCooldown.delete(interaction.guild.id)
        return interaction.editReply(`Successfully locked **${displayChannelsToLock}** channels`).catch(() => { interaction.channel.send(`Successfully locked **${displayChannelsToLock}** channels`).catch(() => {}) })
