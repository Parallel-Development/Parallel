...
module.exports = {
    ...
    async execute(client, interaction, args) {
        ...
        if (!displayName) return interaction.reply({ content: 'Please specify a display name to search for!', ephmeral: true });
        ...
        .setAuthor(`Results for ${displayName} ${exact ? '| The results shown are for users with the exact display name as specified' : ''}`, client.user.displayAvatarURL())

        const _members = [...interaction.guild.members.cache.values()].filter(m => m.displayName.toLowerCase().includes(displayName.toLowerCase()));
        for (let i = 0; i !== _members.length; ++i) {
            const member = _members[i];
            if (exact) {
                if (member.displayName.toLowerCase() === displayName.toLowerCase()) {
                    result.addField(member.user.tag, `ID: \`${member.id}\`\nUsername: \`${member.user.username}\`\nDisplay Name: \`${member.displayName}\``);
                    members.push(member);
                }
            } else {
