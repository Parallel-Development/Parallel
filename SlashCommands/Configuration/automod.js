...
module.exports = {
    ...
    data: new SlashCommandBuilder().setName('automod').setDescription('Manage the auto-moderation for the bot')
    .addSubcommand(command => command.setName('current').setDescription('View the current automod state'))
    .addSubcommand(command => command.setName('filter').setDescription('Manage the filter automod on the server')
        .addStringOption(option => option.setName('punishment').setDescription('The punishment given for triggering this automod').setRequired(true)
            .addChoice('Warn', 'warn')
            .addChoice('Kick', 'kick')
            .addChoice('Mute', 'mute')
            .addChoice('Ban', 'ban')
    ...
    .addSubcommand(command => command.setName('spam').setDescription('Manage the spam automod on the server')
        .addStringOption(option => option.setName('punishment').setDescription('The punishment given for triggering this automod').setRequired(true)
            .addChoice('Warn', 'warn')
            .addChoice('Kick', 'kick')
            .addChoice('Mute', 'mute')
            ...
            .addChoice('Disable', 'disable'))
        .addStringOption(option => option.setName('duration').setDescription('The duration of the punishment')))
    .addSubcommand(command => command.setName('walltext').setDescription('Manage the walltext automod on the server')
        .addStringOption(option => option.setName('punishment').setDescription('The punishment given for triggering this automod').setRequired(true)
            .addChoice('Warn', 'warn')
            .addChoice('Kick', 'kick')
            .addChoice('Mute', 'mute')
            ...
            .addChoice('Disable', 'disable'))
        .addStringOption(option => option.setName('duration').setDescription('The duration of the punishment')))
    .addSubcommand(command => command.setName('links').setDescription('Manage the link automod on the server')
        .addStringOption(option => option.setName('punishment').setDescription('The punishment given for triggering this automod').setRequired(true)
            .addChoice('Warn', 'warn')
            .addChoice('Kick', 'kick')
            .addChoice('Mute', 'mute')
            ...
            .addChoice('Disable', 'disable'))
        .addStringOption(option => option.setName('duration').setDescription('The duration of the punishment')))
    .addSubcommand(command => command.setName('invites').setDescription('Manage the invite automod on the server')
        .addStringOption(option => option.setName('punishment').setDescription('The punishment given for triggering this automod').setRequired(true)
            .addChoice('Warn', 'warn')
            .addChoice('Kick', 'kick')
            .addChoice('Mute', 'mute')
            ...
            .addChoice('Disable', 'disable'))
        .addStringOption(option => option.setName('duration').setDescription('The duration of the punishment')))
    .addSubcommand(command => command.setName('mass-mention').setDescription('Manage the mass-mention automod on the server')
        .addStringOption(option => option.setName('punishment').setDescription('The punishment given for triggering this automod').setRequired(true)
            .addChoice('Warn', 'warn')
            .addChoice('Kick', 'kick')
            .addChoice('Mute', 'mute')
            ...
            .addChoice('Disable', 'disable'))
        .addStringOption(option => option.setName('duration').setDescription('The duration of the punishment')))
    .addSubcommand(command => command.setName('filter-list').setDescription('Manage the filtered words on the server')
        .addStringOption(option => option.setName('method').setDescription('To add, remove, remove all, or view').setRequired(true)
            .addChoice('Add', 'add')
            .addChoice('Remove', 'remove')
            .addChoice('Remove All', 'removeall')
            .addChoice('View', 'view'))
        .addStringOption(option => option.setName('word').setDescription('The word to add or remove from the filter list')))
    .addSubcommand(command => command.setName('channel-bypass').setDescription('Manage the channels that are on the bypass list for automod')
        .addStringOption(option => option.setName('method').setDescription('To add, remove, remove all, or to view').setRequired(true)
            .addChoice('Add', 'add')
            .addChoice('Remove', 'remove')
            .addChoice('Remove All', 'removeall')
            .addChoice('View', 'view'))
        .addChannelOption(option => option.setName('channel').setDescription('The channel to add or remove from the channel bypass')))
    .addSubcommand(command => command.setName('role-bypass').setDescription('Manage the roles that are on the bypass list for automod')
        .addStringOption(option => option.setName('method').setDescription('To add, remove, remove all, or to view').setRequired(true)
            .addChoice('Add', 'add')
            .addChoice('Remove', 'remove')
            .addChoice('Remove All', 'removeall')
            .addChoice('View', 'view'))
        .addRoleOption(option => option.setName('role').setDescription('The role to add or remove from the channel bypass'))
    ),
    async execute(client, interaction, args) {
        ...
        const automodList = new Discord.MessageEmbed()
            ...
            .setDescription('Syntax: `automod (setting) [punishment]` or `automod (setting) disable` to disable. For lists, the options include `add`, `remove`, `removeall`, and `view`\n\nThe valid punishments are `delete`, `warn`, `kick`, `mute`, `tempmute`, and `tempban`')
            .addField('Filter', `Toggled: \`${filterTempMuteDuration ? filter + ' for ' + client.util.duration(filterTempMuteDuration) : filterTempBanDuration ? filter + ' for ' + client.util.duration(filterTempBanDuration) : filter}\``, true)
            .addField('Spam', `Toggled: \`${fastTempMuteDuration ? fast + ' for ' + client.util.duration(fastTempMuteDuration) : fastTempBanDuration ? fast + ' for ' + client.util.duration(fastTempBanDuration) : fast}\``, true)
            .addField('Walltext', `Toggled: \`${walltextTempMuteDuration ? walltext + ' for ' + client.util.duration(walltextTempMuteDuration) : walltextTempBanDuration ? walltext + ' for ' + client.util.duration(walltextTempBanDuration) : walltext}\``, true)
            .addField('Links', `Toggled: \`${linksTempMuteDuration ? links + ' for ' + client.util.duration(linksTempMuteDuration) : linksTempBanDuration ? links + ' for ' + client.util.duration(linksTempBanDuration) : links}\``, true)
            .addField('Invites', `Toggled: \`${invitesTempMuteDuration ? invites + ' for ' + client.util.duration(invitesTempMuteDuration) : invitesTempBanDuration ? invites + ' for ' + client.util.duration(invitesTempBanDuration) : invites}\``, true)
            .addField('Mass-mention', `Toggled: \`${massmentionTempMuteDuration ? massmention + ' for ' + client.util.duration(massmentionTempMuteDuration) : massmentionTempBanDuration ? massmention + ' for ' + client.util.duration(massmentionTempBanDuration) : massmention}\``, true)
            .addField('Filter-list', 'The list of blacklisted words on the server that if used and the Filter automod is enabled, will be moderated', true)
        ...
        switch (_subArgs) {
            case 'filter':
                ...
                if(!duration && toggle['duration']) return client.util.throwError(interaction, client.config.invalid_duration);
                if(duration > 315576000000) return client.util.throwError(interaction, client.config.time_too_long);

                switch (toggle['punishment']) {
                    case 'delete':
                        updateFilter('delete')
                        interaction.reply(`Members who send words on the \`Filtered List\` will get their message deleted`)
                        break;
                    case 'warn':
                        updateFilter('warn')
                    ...
                    case 'tempban':
                        ...
                        interaction.reply(`Members who send words on the \`Filtered List\` will get banned for \`${client.util.duration(duration)}\``)
                        break;
                    case 'tempmute':
                        updateFilter('tempmute', duration)
                        interaction.reply(`Members who send words on the \`Filtered List\` will get muted for  \`${client.util.duration(duration)}\``)
                        break;
                    case 'disable':
            ...
            case 'filter-list':
                ...
                if ((toggle['method'] !== 'view' && toggle['method'] !== 'removeall') && !word) return client.util.throwError(interaction, 'Please specify a word');
                ...
                switch (toggle['method']) {
                    case 'add':
                        ...
                        if (wordAlreadyInFilter?.length !== 0) return interaction.reply('This word is already in the filter! Run `automod filter view` to view the current list of filtered words')
                        await automodSchema.updateOne({
                            guildID: interaction.guild.id
                        },
                            {
                    ...
                    case 'remove':
                        ...
                        if (!wordNotInFilter || !wordNotInFilter.length) return interaction.reply(`Could not find the word \`${word}\` on the filter. Run \`automod filterlist view\` to view the current list of filtered words`)
                        await automodSchema.updateOne({
                            guildID: interaction.guild.id
                        },
                            {
                    ...
                    case 'view':
                        ...
                        if (!filterList || !filterList.length) return interaction.reply('No words are on the filter! Want to add some? `automod filterlist add (word)`')
                        const filterViewList = new Discord.MessageEmbed()
                            ...
                            .setDescription(`${filterList.map(word => `\`${word}\``).join(', ').length <= 1024 ? filterList.map(word => `\`${word}\``).join(', ') : await client.util.createBin(filterList.join(', '))}`)
                ...
                switch (toggle['punishment']) {
                    ...
                    case 'tempban':
                        ...
                        interaction.reply({ embeds: [
                            new Discord.MessageEmbed()
                                .setColor(client.config.colors.main)
                                .setDescription(`✅ Members who send fast message spam will get banned for \`${client.util.duration(duration)}\``)
                                .setAuthor('Automod Update', client.user.displayAvatarURL())
                        ]})
                        break;
                    case 'tempmute':
                        ...
                        interaction.reply({
                            embeds: [
                                new Discord.MessageEmbed()
                                    .setColor(client.config.colors.main)
                                    .setDescription(`✅ Members who send fast message spam will get muted for \`${client.util.duration(duration)}\``)
                                    .setAuthor('Automod Update', client.user.displayAvatarURL())
                            ]
                        })
                        break;
                    case 'disable':
                        ...
                        interaction.reply({ embeds: [
                            new Discord.MessageEmbed({})
                                .setColor(client.config.colors.main)
                                .setDescription(`✅ Members who send fast message spam will no longer be automatically punished`)
                                .setAuthor('Automod Update', client.user.displayAvatarURL())
                        ] })
                        break;
                }
                ...
                switch (toggle['punishment']) {
                    ...
                    case 'tempban':
                        ...
                        interaction.reply({ embeds: [
                            new Discord.MessageEmbed()
                            .setColor(client.config.colors.main)
                            .setDescription(`✅ Members who send walltext will get banned for \`${client.util.duration(duration)}\``)
                            .setAuthor('Automod Update', client.user.displayAvatarURL())
                        ]})
                        break;
                    case 'tempmute':
                        ...
                        interaction.reply({ embeds: [
                            new Discord.MessageEmbed()
                            .setColor(client.config.colors.main)
                            .setDescription(`✅ Members who send walltext will get muted for \`${client.util.duration(duration)}\``)
                            .setAuthor('Automod Update', client.user.displayAvatarURL())
                        ]})
                        break;
                    case 'disable':
                ...
                switch (toggle['punishment']) {
                    ...
                    case 'tempban':
                        ...
                        interaction.reply({ embeds: [
                            new Discord.MessageEmbed()
                            .setColor(client.config.colors.main)
                            .setDescription(`✅ Members who send links will get banned for \`${client.util.duration(duration)}\``)
                            .setAuthor('Automod Update', client.user.displayAvatarURL())
                    ]})
                        break;
                    case 'tempmute':
                        ...
                        interaction.reply({ embeds: [
                            new Discord.MessageEmbed()
                            .setColor(client.config.colors.main)
                            .setDescription(`✅ Members who send links will get muted for \`${client.util.duration(duration)}\``)
                            .setAuthor('Automod Update', client.user.displayAvatarURL())
                    ]})
                        break;
                    case 'disable':
                        ...
                        interaction.reply({ embeds: [
                            new Discord.MessageEmbed()
                            .setColor(client.config.colors.main)
                            .setDescription(`✅ Members who send links will no longer be automatically punished`)
                            .setAuthor('Automod Update', client.user.displayAvatarURL())
                    ]})
                        break;
                }
                ...
                switch (toggle['punishment']) {
                    ...
                    case 'tempban':
                        ...
                        interaction.reply({ embeds: [
                            new Discord.MessageEmbed()
                            .setColor(client.config.colors.main)
                            .setDescription(`✅ Members who send invites will get banned for \`${client.util.duration(duration)}\``)
                            .setAuthor('Automod Update', client.user.displayAvatarURL())
                    ]})
                    ...
                    case 'tempmute':
                        ...
                        interaction.reply({ embeds: [
                            new Discord.MessageEmbed()
                            .setColor(client.config.colors.main)
                            .setDescription(`✅ Members who send invites will get muted for \`${client.util.duration(duration)}\``)
                            .setAuthor('Automod Update', client.user.displayAvatarURL())
                    ]})
                        interaction.reply(success)
                        break;
                ...
                switch (toggle['punishment']) {
                    ...
                    case 'tempban':
                        ...
                        interaction.reply({ embeds: [
                            new Discord.MessageEmbed()
                            .setColor(client.config.colors.main)
                            .setDescription(`✅ Members who mention 5+ users will get banned for \`${client.util.duration(duration)}\``)
                            .setAuthor('Automod Update', client.user.displayAvatarURL())
                    ]})
                        break;
                    case 'tempmute':
                        ...
                        interaction.reply({ embeds: [
                            new Discord.MessageEmbed()
                            .setColor(client.config.colors.main)
                            .setDescription(`✅ Members who mention 5+ users will get muted for \`${client.util.duration(duration)}\``)
                            .setAuthor('Automod Update', client.user.displayAvatarURL())
                    ]})
                        break;
                    case 'disable':
            ...
            case 'channel-bypass':
                ...
                if(bypassChannel && !bypassChannel.isText()) return client.util.throwError(interaction, client.config.errors.not_type_text_channel);


                switch (toggle['method']) {
                    case 'add':
                        if (!bypassChannel) return client.util.throwError(interaction, client.config.errors.missing_argument_channel)
                        if (!bypassChannel.isText()) return client.util.throwError(interaction, client.config.errors.not_type_text_channel)
                        const alreadyInBypassList0 = await automodSchema.findOne({
                            guildID: interaction.guild.id,
                            bypassChannels: bypassChannel.id
                        })
                        if (alreadyInBypassList0?.bypassChannels?.length) return interaction.reply('This channel is already in the bypass list! You can view the list by running `automod bypass view`')
                        await automodSchema.updateOne({
                            guildID: interaction.guild.id
                        },
                            {
                    ...
                    case 'remove':
                        ...
                        if (!alreadyInBypassList1?.bypassChannels?.length) return interaction.reply('This channel is not in the bypass list! You can view the list by running `automod bypass view`')
                        await automodSchema.updateOne({
                            guildID: interaction.guild.id
                        },
                            {
                    ...
                    case 'view':
                        ...
                        if (!bypassChannels || !bypassChannels.length) return interaction.reply('No channels are on the automod bypass list! Want to add some? `automod bypass add (channel)`')
                        const bypassChannelsViewList = new Discord.MessageEmbed()
                            .setColor(client.config.colors.main)
                            .setAuthor(`Bypassed channel list for ${interaction.guild.name}`, client.user.displayAvatarURL())
                        for (let i = 0; i !== bypassChannels.length; ++i) {
                            const channel = bypassChannels[i];
                            if (!interaction.guild.channels.cache.get(channel) || !interaction.guild.channels.cache.get(channel).isText()) {
                                await automodSchema.updateOne({
                                    guildID: interaction.guild.id
                                },
                                    {
                        ...
                        const bypassChannels_ = bypassChannels.map(channel => interaction.guild.channels.cache.get(channel)).filter(channel => channel.isText()).join(', ');
            ...
            case 'role-bypass':
                ...
                switch (toggle['method']) {
                    case 'add':
                        ...
                        if (alreadyInBypassList0?.bypassRoles?.length) return interaction.reply('This role is already in the bypass list! You can view the list by running `automod rolebypass view`')
                        await automodSchema.updateOne({
                            guildID: interaction.guild.id
                        },
                            {
                    ...
                    case 'remove':
                        ...
                        if (!alreadyInBypassList1?.bypassRoles?.length) return interaction.reply('This role is not in the bypass list! You can view the list by running `automod rolebypass view`')
                        await automodSchema.updateOne({
                            guildID: interaction.guild.id
                        },
                            {
                    ...
                    case 'view':
                        ...
                        if (!bypassRoles || !bypassRoles.length) return interaction.reply('No roles are on the automod rolebypass list! Want to add some? `automod rolebypass add (role)`')
                        const bypassRolesViewList = new Discord.MessageEmbed()
                            .setColor(client.config.colors.main)
                            .setAuthor(`Bypassed role list for ${interaction.guild.name}`, client.user.displayAvatarURL())
                        const bypassRoles2 = [];
