...
module.exports = {
    ...
    data: new SlashCommandBuilder().setName('punishment-system').setDescription('Punishments to be automatically distributed to users reaching a specific amount of automod warnings')
    .addSubcommand(command => command.setName('set').setDescription('Set a specific warning instance to a punishment')
        .addIntegerOption(option => option.setName('warning_count').setDescription('The amount of automod warnings a user should reach to get punished').setRequired(true))
    ...
    .addSubcommand(command => command.setName('disable').setDescription('Remove a punishment system warning count instance')
        .addIntegerOption(option => option.setName('warning_count').setDescription('The warning count').setRequired(true)))
    .addSubcommand(command => command.setName('reset').setDescription('Remove all warning count instances from the punishment system'))
    ...
    async execute(client, interaction, args) {
        ...
        if (subArgs['set']) {
            ...
            switch (punishment) {
                ...
                case 'kick':
                    if (systemWarningCountCheck?.system?.length > 0) return client.util.throwError(interaction, `A punishment is already given out at this warning count. Please disable this first then try again`)
                ...
                case 'tempmute':
                    ...
                    interaction.reply(`Success! Users will now get muted for \`${client.util.duration(duration)}\` for reaching ${warningCount} warnings`)
                    break;
                case 'tempban':
                    if (systemWarningCountCheck?.system?.length > 0) return client.util.throwError(interaction, `A punishment is already given out at this warning count. Please disable this first then try again`);
                    updateSystemSchema('tempban', duration)
                    interaction.reply(`Success! Users will now get banned for \`${client.util.duration(duration)}\` for reaching ${warningCount} warnings`)
                    break;
                default:
                return client.util.throwError(interaction, 'Invalid punishment! The punishments are: `kick, mute, ban, tempmute, tempban` or `disable` to remove the punishment given at the specified warning count')
            }
            ...
            if (!systemWarningCountCheck || systemWarningCountCheck.system.length === 0) return client.util.throwError(interaction, 'There is no punishment given at this warning count. You can view the current system by running `punishment-system view`');
            await systemSchema.updateOne({
                guildID: interaction.guild.id
            },
            {
