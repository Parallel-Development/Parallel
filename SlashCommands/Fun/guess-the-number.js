...
module.exports = {
    ...
    data: new SlashCommandBuilder().setName('guess-the-number').setDescription('A minigame of Guess the Number!')
    ...
    async execute(client, interaction, args) {
        if (openedSession.has(interaction.user.id)) return interaction.reply({ content: 'You already have an opened game ', ephemeral: true });
        ...
        let answered = false;
        interaction.reply(`A number has been chosen \`0-1000\`. You have ${`\`${max}\` ${max === 1 ? 'try' : 'tries'}`} and \`${client.util.duration(time)}\` to guess the number! (You can run \`cancel\` to cancel this minigame)`);
        const filter = m => m.author.id === interaction.user.id;
        const collector = new Discord.MessageCollector(interaction.channel, { time: time, filter: filter, max: max });
        collector.on('collect', async(message) => {

            if (cooldown.has(message.author.id)) return interaction.react('🕑')
            else {
