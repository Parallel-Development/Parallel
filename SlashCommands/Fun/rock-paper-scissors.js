...
module.exports = {
    ...
    data: new SlashCommandBuilder().setName('rock-paper-scissors').setDescription('Play someone in a minigame of rock paper scissors')
    .addUserOption(option => option.setName('member').setDescription('The member to play against').setRequired(true)),
    async execute(client, interaction, args) {

        if (global.openedSession.has(interaction.user.id)) return client.util.throwError(interaction, 'You are already in a game!')
        ...
        if (global.requestCooldown.has(interaction.user.id)) return client.util.throwError(interaction, 'You already have a pending request! Please wait for it to expire before trying again');
        if (global.requestedCooldown.has(playerTwo.id)) return client.util.throwError(interaction, 'This user already has a pending request! Please wait for their pending request to expire before trying again');
        ...
        if (playerTwo.id === interaction.user.id) return client.util.throwError(interaction, 'You cannot play yourself');
        ...
        interaction.reply({ content: `${playerTwo}, ${playerOne} would like to play you in rock-paper-scissors | To play, hit the \`Play\` button below`, components: [join] });
