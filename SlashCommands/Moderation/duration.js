...
module.exports = {
    ...
    data: new SlashCommandBuilder().setName('duration').setDescription('Change the duration of a punishment')
    .addStringOption(option => option.setName('id').setDescription('The ID of the punishment to change the duration of').setRequired(true))
    .addStringOption(option => option.setName('new_duration').setDescription('The new duration to set the punishment to').setRequired(true)),
    ...
    async execute(client, interaction, args) {
        ...
        return interaction.reply(`Successfully updated duration for punishment \`${punishmentID}\` to \`${client.util.duration(ms(newDuration))}\``)
    }
}