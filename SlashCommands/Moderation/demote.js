...
module.exports = {
    ...
    data: new SlashCommandBuilder().setName('demote').setDescription('Remove all roles with staff permissions from a member')
    .addUserOption(option => option.setName('member').setDescription('The member to remove all roles with staff permissions from').setRequired(true))
    .addBooleanOption(option => option.setName('dm').setDescription('DM the user regarding their demotion (with if specified, your reason)'))
    .addStringOption(option => option.setName('reason').setDescription('The message sent to the user regarding their demote if you are to DM them')),
    ...
    async execute(client, interaction, args) {
        ...
        if (member.roles.cache.size === memberRoles.length) return client.util.throwError(interaction, 'This user does not have any roles with staff permissions');
        ...
        if (args['dm'] === true) {
            ...
            await member.send({ embeds: [demotedDMEmbed] }).catch(() => { failedToSend = true })
        }

        const demotedEmbed = new Discord.MessageEmbed()
        .setColor(client.config.colors.punishment[1])
        .setDescription(`✅ ${member} has been demoted ${args['dm'] ? failedToSend ? '| Failed to DM them' : '| Successfully DM\'d them' : ''}`)
