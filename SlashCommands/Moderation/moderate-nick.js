...
module.exports = {
    ...
    data: new SlashCommandBuilder().setName('moderate-nick').setDescription('Set the nickname of a user to Moderated_(Random Code)')
    ...
    .addStringOption(option => option.setName('reason').setDescription('The message sent to the user regarding their nickname change if you are to DM them')),
    ...
    async execute(client, interaction, args) {
        ...
        if (args['dm'] === true) {
            ...
            await member.send(`Your username was moderated in **${interaction.guild.name}** ${reason ? "| Reason: " + reason : ""}`).catch(() => failedtoSend = true )
        }

        const moderatedNicknameEmbed = new Discord.MessageEmbed()
        .setColor(client.config.colors.main)
        .setDescription(`✅ User with ID \`${member.id}\` has been moderated with identifier code \`${code}\` ${args['dm'] ? failedToSend ? '| Failed to DM them' : '| Successfully DM\'d them' : ''}`)
