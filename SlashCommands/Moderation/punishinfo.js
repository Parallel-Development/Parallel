...
module.exports = {
    ...
    data: new SlashCommandBuilder().setName('punishinfo').setDescription('View more in depth detail of a punishment')
    .addStringOption(option => option.setName('id').setDescription('The punishment ID to get information of').setRequired(true)),
    ...
    async execute(client, interaction, args) {
        ...
        if (duration && (type !== 'Unmute' || type === 'Kick')) punishInfoEmbed.addField('Expires', duration !== 'Permanent' ? expires - Date.now() > 0 ? client.util.timestamp(expires) : 'This punishment has already expired' : 'Never');
