...
module.exports = {
    ...
    data: new SlashCommandBuilder().setName('shortcut').setDescription('Access a server shortcut using this command')
    ...
    async execute(client, interaction, args) {
        ...
        if (!interaction.guild.me.permissions.has(permissions) && permissions !== 'warn') return missingPerms(permissions.replace(pemissions.toUpperCase().replace('_', ' ').replace('_', ' ')));
        ...
        if (shortcmd.type === 'mute' || shortcmd.type === 'tempmute' || shortcmd.type === 'ban' || shortcmd.type === 'tempban') {
            ...
            if (shortcmd.type === 'ban' || shortcmd.type === 'tempban') {
                ...
                new Infraction(client, 'Ban', interaction, interaction.member, member, { reason: shortcmd.reason, punishmentID: punishmentID, time: duration, auto: false });
                if (shortcmd.type === 'tempban') new Punishment(interaction.guild.name, interaction.guild.id, 'ban', member.id, { reason: shortcmd.reason, time: duration ? Date.now() + duration : 'Never' });
                ...
                new Infraction(client, 'Mute', interaction, interaction.member, member, { reason: shortcmd.reason, punishmentID: punishmentID, time: duration, auto: false });
                new Punishment(interaction.guild.name, interaction.guild.id, 'mute', member.id, { reason: shortcmd.reason, time: duration ? Date.now() + duration : 'Never', roles: memberRoles });
                new DMUserInfraction(client, 'muted', client.config.colors.punishment[1], interaction, member, { reason: shortcmd.reason, punishmentID: punishmentID, time: duration });
                new ModerationLogger(client, 'Muted', interaction.member, member, interaction.channel, { reason: shortcmd.reason, duration: duration, punishmentID: punishmentID });
            }
        } else {
            ...
            if (shortcmd.type === 'kick') {
                ...
                new Infraction(client, 'Kick', interaction, interaction.member, member, { reason: shortcmd.reason, punishmentID: punishmentID, time: null, auto: false });
                new ModerationLogger(client, 'Kicked', interaction.member, member, interaction.channel, { reason: shortcmd.reason, duration: null, punishmentID: punishmentID })
            } else if (shortcmd.type === 'warn') {
                new Infraction(client, 'Warn', interaction, interaction.member, member, { reason: shortcmd.reason, punishmentID: punishmentID, time: shortcmd.duration, auto: false });
                new DMUserInfraction(client, 'warned', client.config.colors.punishment[1], interaction, member, { reason: shortcmd.reason, punishmentID: punishmentID, time: shortcmd.duration })
                new ModerationLogger(client, 'Warned', interaction.member, member, interaction.channel, { reason: shortcmd.reason, duration: shortcmd.duration, punishmentID: punishmentID })
            }
        }
