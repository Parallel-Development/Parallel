...
module.exports = {
    ...
    data: new SlashCommandBuilder()
        ...
        .setDescription('Clear all infractions from a user')
        .addUserOption(option =>
            option.setName('user').setDescription('The user to clear all infractions from').setRequired(true)
        ),
    ...
    async execute(client, interaction, args) {
        ...
        if (!guildWarnings?.warnings?.length) return interaction.reply(`This user has no infractions`);
        ...
        global.confirmationRequests.push({
            ...
            guildID: interaction.guild.id,
            request: 'clearInfractions',
            at: Date.now(),
            data: { ID: user.id }
        });
        return interaction.reply(
            `Are you sure? This will delete all warnings from **${user.tag}**. To confirm, run \`/confirm\`. To cancel, run \`/cancel\``
        );
    }
};
