...
module.exports = {
    ...
    data: new SlashCommandBuilder().setName('infractions').setDescription('View the infractions of a member or yourself')
    ...
    async execute(client, interaction, args) {
        ...
        if (member !== interaction.user && !interaction.member.permissions.has(Discord.Permissions.FLAGS.MANAGE_MESSAGES)) return client.util.throwError(interaction, 'You do not have the permission to view the infractions of other members');
        ...
        if (!userWarnings?.length) return interaction.reply(`${member === interaction.user ? 'You have' : 'This user has'} no infractions!`);
        ...
        if (pageNumber > amountOfPages) {
            return client.util.throwError(interaction, `Please specify a page number between \`1\` and \`${amountOfPages}\``)
