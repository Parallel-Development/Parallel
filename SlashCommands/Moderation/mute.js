...
module.exports = {
    ...
    data: new SlashCommandBuilder().setName('mute').setDescription('Mutes a member denying their permission to speak in the rest of the server')
    ...
    async execute(client, interaction, args) {
        ...
        if (member && member.permissions.has(Discord.Permissions.FLAGS.MANAGE_ROLES) && !removerolesonmute) return client.util.throwError(message, 'This command may not be effective on this member | If you have the **Remove Roles On Mute** module enabled, this may work');
        ...
        if (!member) {
            ...
            new Infraction(client, 'Mute', interaction, interaction.member, user, { reason: reason, punishmentID: punishmentID, time: time, auto: false });
            new Punishment(interaction.guild.name, interaction.guild.id, 'mute', user.id, { reason: reason, time: time ? Date.now() + time : 'Never' });
            new ModerationLogger(client, 'Muted', interaction.member, user, interaction.channel, { reason: reason, duration: time, punishmentID: punishmentID });

            return interaction.reply(`**${user.tag}** has been muted. They are not currently on the server, but when they rejoin they will be muted`);
        }
        ...
        if (member.roles.cache.has(role.id)) return client.util.throwError(interaction, 'This user is already currently muted!');
        ...
        new Infraction(client, 'Mute', interaction, interaction.member, member, { reason: reason, punishmentID: punishmentID, time: time, auto: false });
        new Punishment(interaction.guild.name, interaction.guild.id, 'mute', member.id, { reason: reason, time: time ? Date.now() + time : 'Never', roles: memberRoles });
        new DMUserInfraction(client, 'muted', client.config.colors.punishment[1], interaction, member, { reason: reason, punishmentID: punishmentID, time: time })
        new ModerationLogger(client, 'Muted', interaction.member, member, interaction.channel, { reason: reason, duration: time, punishmentID: punishmentID})
