...
module.exports = {
    ...
    data: new SlashCommandBuilder()
        .setName('help')
        .setDescription('Get a list of all commands or get help on a specific command')
...
async function getAll(client, interaction, prefix, shortcutCommands, modRoles) {
    ...
    for (const folder of commandFolders) {
        ...
        for (const file of commandFiles) {
            ...
            if (cmd.developer && client.config.developers.includes(interaction.user.id))
    ...
    const linkInviteLink = new Discord.MessageButton()
    ...
    const linkSupportServer = new Discord.MessageButton()
        .setLabel('Support Server')
        .setStyle('LINK')
        .setURL('https://discord.gg/v2AV3XtnBM');
    const linkDocumentation = new Discord.MessageButton()
        .setLabel('Documentation')
        .setStyle('LINK')
        .setURL('https://piyeris0.gitbook.io/parallel/');
    const linkBotGuidelines = new Discord.MessageButton()
        .setLabel('Bot Guidelines')
        .setStyle('LINK')
        .setURL('https://docs.google.com/document/d/1u0Z6WVS0V8D72E8cU5gUw2OwGeqW3g1OvwSkzG7odOw/edit');
    const linkPrivacyPolicy = new Discord.MessageButton()
...
async function getCMD(client, interaction, input, shortcutCommands, modRoles) {
    const cmd =
        ...
        client.slashCommands.get(client.aliases.get(input.toLowerCase()));
    ...
    if (
        ...
        const sembed = new Discord.MessageEmbed().setColor(client.util.getMainColor(interaction.guild));
        if (scmd.name) sembed.setAuthor(`Help | ${scmd.name}`, client.user.displayAvatarURL());
        if (scmd.type)
            sembed.addField(
                'Punishment Type',
                scmd.type.replace('temp', '')[0].toUpperCase() + scmd.type.replace('temp', '').slice(1)
            );
        if (scmd.reason)
            sembed.addField(
                'Punishment Reason',
                scmd.reason.length >= 1024 ? await client.util.createBin(scmd.reason) : scmd.reason
            );
        if (scmd.duration && scmd.duration !== 'Permanent')
            sembed.addField('Punishment Duration', `${client.util.duration(scmd.duration)}`);
    ...
    if (!cmd || (cmd.developer && !client.config.developers.includes(interaction.user.id)) || cmd.hidden) {
        embed.setColor(client.config.colors.err).setDescription(`No information found for the provided command`);
        return interaction.reply({ embeds: [embed] });
    }

    embed.setColor(client.util.getMainColor(interaction.guild));
