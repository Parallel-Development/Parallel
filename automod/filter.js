...
exports.run = async (client, message) => {
    ...
    let { filter, filterTempBanDuration, filterTempMuteDuration } = automodGrab
    ...
    if (filter == 'warn') {
        ...
        if (autowarnexpire !== 'disable') {
            caseInfo = {
                ...
                type: 'Warn',
                date: date,
                reason: '[PARALLEL FILTER DETECTION] Sending Filtered Words',
                code: code,
        ...
        const userwarned = new Discord.MessageEmbed()
            .setColor('#09fff2')
            .setDescription(`${message.member} has been warned with ID \`${code}\`for using filtered words <a:check:800062847974375424> `)
        ...
        const warndm = new Discord.MessageEmbed()
            ...
            .addField('Reason', '[PARALLEL FILTER DETECTION] Using Filtered Words')
    ...
    if (filter == 'kick') {
        ...
        const userkicked = new Discord.MessageEmbed()
            .setColor('#09fff2')
            .setDescription(`${message.member} has been kicked with ID \`${code}\`for using filtered words <a:check:800062847974375424> `)
        ...
        message.member.kick('[PARALLEL FILTER DETECTION] Using Filtered Words').catch(() => { return })

        const caseInfo = {
            moderatorID: message.guild.me.id,
            type: 'Kick',
            date: date,
            reason: '[PARALLEL FILTER DETECTION] Using Filtered Words',
            code: code,
            auto: true
        }
    ...
    if (filter == 'mute') {
        ...
        const usermuted = new Discord.MessageEmbed()
            .setColor('#09fff2')
            .setDescription(`${message.member} has been muted with ID \`${code}\`for using filtered words <a:check:800062847974375424> `)
    ...
    if (filter == 'ban') {
        ...
        const userbanned = new Discord.MessageEmbed()
            .setColor('#09fff2')
            .setDescription(`${message.member} has been banned with ID \`${code}\`for using filtered words <a:check:800062847974375424> `)
        ...
        message.guild.members.ban(message.member, { reason: '[PARALLEL FILTER DETECTION] Using Filtered Words ' }).catch(() => { return })
    ...
    if (filter == 'tempban') {
        ...
        var file = require('../structures/automodLogging');
        file.run(client, 'Banned', message.member, message.channel, caseInfo.reason, cleanTime(filterTempBanDuration), caseInfo.code)
    }
    ...
    if (filter == 'tempmute') {
        ...
        var file = require('../structures/automodLogging');
        file.run(client, 'Muted', message.member, message.channel, caseInfo.reason, cleanTime(filterTempMuteDuration), caseInfo.code)
    }
}
