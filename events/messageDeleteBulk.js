...
module.exports = {
    ...
    async execute(client, messages) {
        ...
        const sortedMessages = [...messages.values()]
            .reverse()
            .map(message => `${message?.author.tag || 'Unknown'}: ${message?.content || 'No content provided'}`)
            .join('\n');
        ...
        const logEmbed = new Discord.MessageEmbed()
            ...
            .addField(
                'Purged Messages',
                sortedMessages.length >= 1024 ? await client.util.createBin(sortedMessages) : sortedMessages
            );

        const binContent = [...messages.values()]
            .filter(message => message.content || message.attachments.size)
            .reverse()
            .map(
                message =>
                    `[ ${moment.utc(message.createdTimestamp).format('h:mm:ss A')} ] from "${
                        message.author.tag
                    }" with ID ${message.author.id}:${message.content ? `\n- Content: ${message.content}` : ''}${
        ...
        const bin = await client.util.createBin(
            binContent
                ? `// Note: all listed times are in UTC\n\n${binContent}`
                : 'No detailed message data could be logged'
        );
        ...
        return logChannel.send({ embeds: [logEmbed], components: [row] }).catch(() => {});
    }
};
