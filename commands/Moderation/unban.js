...
module.exports = {
    ...
    async execute(client, message, args) {
        ...
        const serverBans = await message.guild.fetchBans();
        if(!serverBans.size) return message.channel.send('There are no users banned from this server')
        const userBanned = await message.guild.fetchBans().then(bans => bans.find(ban => ban.user.id === user.id));
        if(!userBanned) return message.channel.send('This user is not banned');
        ...
        return message.channel.send(unbannedEmbed);
