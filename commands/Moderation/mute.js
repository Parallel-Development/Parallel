...
module.exports = {
    ...
    async execute(client, message, args) {
        ...
        if (!member && await client.util.getUser(client, args[0])) {
            ...
            new Infraction(client, 'Mute', message, message.member, user, { reason: reason, punishmentID: punishmentID, time: time, auto: false });
            new Punishment(message.guild.name, message.guild.id, 'mute', user.id, { reason: reason, time: time ? Date.now() + time : 'Never' });
            new ModerationLogger(client, 'Muted', message.member, user, message.channel, { reason: reason, duration: time, punishmentID: punishmentID });
        ...
        if (member.roles.cache.has(role.id)) return client.util.throwError(message, 'This user is already currently muted!');
        ...
        new Punishment(message.guild.name, message.guild.id, 'mute', member.id, { reason: reason, time: time ? Date.now() + time : 'Never', roles: memberRoles });
        new DMUserInfraction(client, 'muted', client.config.colors.punishment[1], message, member, { reason: reason, punishmentID: punishmentID, time: time })
        new ModerationLogger(client, 'Muted', message.member, member, message.channel, { reason: reason, duration: time, punishmentID: punishmentID})
