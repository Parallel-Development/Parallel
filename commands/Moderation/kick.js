...
module.exports = {
    ...
    async execute(client, message, args) {
        ...
        await new DMUserInfraction(client, 'kicked', client.config.colors.punishment[1], message, member, { reason: reason, punishmentID: punishmentID, time: 'ignore' });

        await member.kick(reason);

        new Infraction(client, 'Kick', message, message.member, member, { reason: reason, punishmentID: punishmentID, time: null, auto: false });
        new ModerationLogger(client, 'Kicked', message.member, member, message.channel, { reason: reason, duration: null, punishmentID: punishmentID })
