...
module.exports = {
    ...
    async execute(client, message, args) {
        ...
        if (!channels.length)
            return client.util.throwError(message, 'none of the target channels will change after locked');
                ...
                if (!allOverwrites.length && everyoneRoleOverwrite);
                    ...
                    await channel.permissionOverwrites
                        .set(
                            ...
                            `Command lockserver ran by ${message.author.tag} (${message.author.id})`
                        )
        ...
        global.lockdownCooldown.delete(message.guild.id);
        return msg.edit(`Successfully locked **${displayChannelsToLock}** channels`).catch(() => {
            message.channel.send(`Successfully locked **${displayChannelsToLock}** channels`).catch(() => {});
        });
    }
};
