...
module.exports = {
    ...
    async execute(client, message, args) {
        ...
        const option = args[0];
        if (!(option === 'archiveall' || option === 'deleteall')) return client.util.throwError(message, client.config.errors.invalid_option);
        ...
        if (!channel.permissionsFor(message.member).has(Discord.Permissions.FLAGS.MANAGE_CHANNELS)) return client.util.throwError(message, 'You do not have permission to manage this channel');
