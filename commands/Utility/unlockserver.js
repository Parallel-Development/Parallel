...
module.exports = {
    ...
    async execute(client, message, args) {
        ...
        for (let i = 0; i !== channels.length; ++i) {
            ...
            if (
                !client.guilds.cache.get(message.guild.id).me.permissions.has(Discord.Permissions.FLAGS.ADMINISTRATOR)
            ) {
                msg.edit(
                    'The unlocking process has been automatically halted! I require Administrator to ensure I can update channel overrides. Please give me the administrator permission and run the command again'
                ).catch(() =>
                    message.channel
                        .send(
                            'The unlocking process has been automatically halted! I require Administrator to ensure I can update channel overrides. Please give me the administrator permission and run the command again'
            ...
            if (channel.type === 'number' || !getLockSchema) {
                ...
                let newPermissionOverwrites = permissionOverwrites.filter(
                    overwrite =>
                ...
                await channel.permissionOverwrites
                    .set(
                        ...
                        `Command unlockserver ran by ${message.author.tag} (${message.author.id})`
                    )
                    .catch(async () => await msg.edit('Hmm...'));
        ...
        global.lockdownCooldown.delete(message.guild.id);
        return msg.edit(`Successfully unlocked **${displayChannelsToLock}** channels`).catch(() => {
            message.channel.send(`Successfully unlocked **${displayChannelsToLock}** channels`).catch(() => {});
        });
    }
};
