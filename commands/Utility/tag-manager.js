    ...
    async execute(client, message, args) {
        ...
        if (option === 'create') {
            ...
            if (tagText.length > 2000)
                return message.reply('Error: the tag text length exceeded the limit of **2000** characters');
            ...
            return message.reply(
                `Are you sure? This will remove all all tags in this server. To confirm, run the \`confirm\` command. To cancel, run the \`cancel\` command`
            );
        } else if (option === 'edit') {
            const tagName = args[1];
            const validateTag = await tagSchema.findOne({
            ...
            if (args[1] === 'add') {
                ...
                if (allowedRoleList.includes(role.id))
                    return client.util.throwError(message, 'this role is already on the allowed role list');
                ...
                if (!role) return client.util.throwError(message, client.config.errors.invalid_role);
                if (!allowedRoleList.includes(role.id))
                    return client.util.throwError(message, 'this role is not on the allowed role list');
