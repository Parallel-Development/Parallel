...
module.exports = {
    ...
    async execute(client, message, args) {
        ...
        const evaluatingMessage = !silent
            ? !delMessage
                ? await message.reply({ embeds: [tryingToEval] })
                : await message.channel.send({ embeds: [tryingToEval] })
            : null;
        ...
        try {
            ...
            if (silent)
                if (!delMessage) return message.reply({ embeds: [error] }).catch(() => {});
                else message.channel.send({ embeds: [error] });
            else
                return evaluatingMessage.edit({ embeds: [error] }).catch(() => {
                    return;
                });
        }
