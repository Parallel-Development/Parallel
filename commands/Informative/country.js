...
module.exports = {
    ...
    async execute(client, message, args) {
        ...
        if(getCode.name) countryInformation.setTitle(getCode.name)
        if(getCode.name) countryInformation.addField('Country Name', getCode.name, true)
        ...
        if(languages.length > 0) countryInformation.addField('Spoken Languages', languages.join(', '), true)
