...
async function getAll(client, message, prefix, shortcutCommands, modRoles) {
    ...
    for (const folder of commandFolders) {
        ...
        for (const file of commandFiles) {
            ...
            if (cmd.developer && client.config.developers.includes(message.author.id))
                commands.push(`\`${path.parse(file).name}\``);
            else if (!cmd.developer && !cmd.hidden) commands.push(`\`${path.parse(file).name}\``);
        }
        if (commands.length !== 0)
            mainHelp.addField(dictionary[folder] ? dictionary[folder] + '  ' + folder : folder, commands.join(', '));
    }
    if (
        shortcutCommands.length &&
        (message.member.permissions.has(Discord.Permissions.FLAGS.MANAGE_MESSAGES) ||
            message.member.roles.cache.some(role => modRoles.includes(role)))
    ) {
        const scommands = [];
...
async function getCMD(client, message, input, shortcutCommands, modRoles) {
    const cmd =
        client.commands.get(input.toLowerCase()) || client.commands.get(client.aliases.get(input.toLowerCase()));
