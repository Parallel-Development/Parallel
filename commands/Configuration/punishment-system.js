...
module.exports = {
    ...
    async execute(client, message, args) {
        ...
        if (warningsCount === 'view') {
            if (systemCheck.system.length === 0) return message.reply('No warning amount instances are set for this server')
            ...
            return message.reply({ embed: [pSystem] });
        ...
        if (warningsCount && !ms(warningsCount)) return message.reply(client.config.errorMessages.bad_input_number);
        warningsCount = Math.round(warningsCount)
        if (warningsCount && warningsCount < 2) return message.reply('The value must be 2 or greater!');
        if (warningsCount && warningsCount > 20) return message.reply('The value must be less than or equal to 20!');
        ...
        if (warningsCount && !punishment) return message.reply(`Please specify what punishment a user should be given for reaching \`${warningsCount}\` warnings`)
        ...
        switch (punishment) {
            ...
            case 'disable':
                if (!systemWarningCountCheck || systemWarningCountCheck.system.length === 0) return message.reply('There is no punishment given at this warning count. You can view the current system by running `punishment-system view`');
                updateSystemSchema('disable');
                message.reply(`Successfully removed the punishment given at \`${warningsCount}\` warnings`)
                break;
            case 'kick':
                if (systemWarningCountCheck && systemWarningCountCheck.system.length > 0) return message.reply(`A punishment is already given out at this warning count. Please disable this first then try again`)
            ...
            case 'tempmute':
                ...
                message.reply(`Success! Users will now get muted for \`${client.util.convertMillisecondsToDuration(duration)}\` for reaching ${warningsCount} warnings`)
                break;
            case 'tempban':
                if (systemWarningCountCheck && systemWarningCountCheck.system.length > 0) return message.reply(`A punishment is already given out at this warning count. Please disable this first then try again`);
                updateSystemSchema('tempban', duration)
                message.reply(`Success! Users will now get banned for \`${client.util.convertMillisecondsToDuration_duration}\` for reaching ${warningsCount} warnings`)
                break;
            default:
                if (warningsCount) return message.reply('Invalid punishment! The punishments are: `kick, mute, ban, tempmute, tempban` or `disable` to remove the punishment given at the specified warning count')
                else {
                    if (systemCheck.system.length === 0) return message.reply('No warning amount instances are set for this server')
