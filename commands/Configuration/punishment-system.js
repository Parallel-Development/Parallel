    ...
    async execute(client, message, args) {
        ...
        switch (punishment) {
            ...
            case 'disable':
                ...
                break;
            case 'kick':
                if (systemWarningCountCheck?.system?.length > 0) return message.reply(`A punishment is already given out at this warning count. Please disable this first then try again`)
            ...
            case 'tempmute':
                ...
                message.reply(`Success! Users will now get muted for \`${client.util.duration(duration)}\` for reaching ${warningsCount} warnings`)
                break;
            case 'tempban':
                if (systemWarningCountCheck?.system?.length > 0) return message.reply(`A punishment is already given out at this warning count. Please disable this first then try again`);
                updateSystemSchema('tempban', duration)
                message.reply(`Success! Users will now get banned for \`${client.util.duration(duration)}\` for reaching ${warningsCount} warnings`)
                break;
            default:
                return message.reply('Invalid punishment! The punishments are: `kick, mute, ban, tempmute, tempban` or `disable` to remove the punishment given at the specified warning count')
        }
    }
}
