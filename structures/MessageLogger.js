...
class MessageLogger {
    ...
    constructor(client, channel, message, oldMessage = null) {
        ...
        if (oldMessage && typeof oldMessage !==  'object') throw new Error('An optional argument `oldMessage` was found, but not as an object')
        ...
        const main = async() => {
            ...
            if (oldMessage) {
                ...
                if (message.content) logEmbed.addField('Deleted Message', message.content.length <= 1024 ? message.content : await client.util.createBin(message.content))
