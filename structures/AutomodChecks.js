...
class AutomodChecks {
    ...
    async execute(onlyCheck = false) {
        ...
        if (client.cache.maliciousLinks.some(url => message.content.includes(url))) {
            let punished = await new Automod(client, message, 'maliciouslinks').execute(onlyCheck);
        ...
        if (
            ...
            let punished = await new Automod(client, message, 'filter').execute(onlyCheck);
            if (punished !== false) return true;
        }
        ...
        if (!edit && message.mentions.users.filter(user => !user.bot).size >= 5) {
            let punished = await new Automod(client, message, 'massmention').execute(onlyCheck);
            if (punished !== false) return true;
        }
        // Walltext
        ...
        if (walltextCheck.length >= 9 || message.content.length >= 700) {
            let punished = new Automod(client, message, 'walltext').execute(onlyCheck);
            if (punished !== false) return true;
        }
        // Spam
        ...
        if (!edit) {
            if (userMap.has(message.author.id)) {
                ...
                if (parseInt(msgCount) === 4) {
                    ...
                    let punished = await new Automod(client, message, 'fast').execute(onlyCheck);
                    if (punished !== false) return true;
                } else {
                    msgCount++;
        ...
        if (inviteCheck.test(message.content.toLowerCase())) {
            let punished = await new Automod(client, message, 'invites').execute(onlyCheck);
            if (punished !== false) return true;
        }
        // Links
