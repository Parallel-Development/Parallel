...
module.exports.run = async (client, interaction) => {
    const gameStarter =
        (await client.util.getMember(interaction.guild, interaction.message.interaction?.user.id)) ||
        interaction.channel.messages.cache.get(interaction.message.reference.messageId).member;
    ...
    if (interaction.user.id !== requested)
    ...
    collector.on('collect', async _interaction => {
        ...
        if (answers.length === 2) {
            const member1 = await client.util.getMember(
                interaction.guild,
                answers.find(answer => answer.ID === gameStarter.user.id).ID
            );
            const member2 = await client.util.getMember(
                interaction.guild,
                answers.find(answer => answer.ID === requested).ID
            );
            ...
            let winner;
            if (member1Option === 'rock' && member2Option === 'scissors')
                (winner = member1), (beats = '**rock** _crushes_ **scissors**');
            if (member1Option === 'rock' && member2Option === 'paper')
                (winner = member2), (beats = '**paper** _covers_ **rock**');
            if (member1Option === 'scissors' && member2Option === 'rock')
                (winner = member2), (beats = '**rock** _crushes_ **scissors**');
            if (member1Option === 'scissors' && member2Option === 'paper')
                (winner = member1), (beats = '**scissors** _cuts_ **paper**');
            if (member1Option === 'paper' && member2Option === 'rock')
                (winner = member1), (beats = '**paper** _covers_ **rock**');
            if (member1Option === 'paper' && member2Option === 'scissors')
                (winner = member2), (beats = '**scissors** _cuts_ **paper**');
